"""
Flux2Klein EMA Inference Script

Inference using EMA-processed transformer weights.

Usage:
    accelerate launch --num_processes=4 test/test_flux2klein_ema_inference.py \
        --ema_checkpoint_dir /path/to/ema_checkpoint \
        --output_dir /path/to/output \
        --experiment_name GA4_EMA
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="EMA Inference")
    parser.add_argument("--ema_checkpoint_dir", type=str, required=True,
                        help="Path to EMA checkpoint directory (containing transformer/)")
    parser.add_argument("--output_dir", type=str, required=True,
                        help="Output directory for generated images")
    parser.add_argument("--experiment_name", type=str, default="EMA",
...
def run_inference(pipeline, validation_prompts, resolution_list, output_dir,
...
def main():
    ...
    transformer_path = os.path.join(args.ema_checkpoint_dir, "transformer")
    if not os.path.exists(transformer_path):
        transformer_path = args.ema_checkpoint_dir
    
    if rank == 0:
        print(f"\n{'='*60}")
        print(f"Flux2Klein EMA Inference - {args.experiment_name}")
        print(f"{'='*60}")
        print(f"  EMA Checkpoint: {transformer_path}")
    ...
    if rank == 0:
        print(f"Total prompts: {len(validation_prompts)}")
    
    try:
        # Serial model loading
        pipeline = None
        for load_rank in range(world_size):
            if rank == load_rank:
                print(f"[Rank {rank}] Loading EMA checkpoint...")
                pipeline = load_pipeline(config, transformer_path, device, weight_dtype)
                print(f"[Rank {rank}] Loaded!")
            state.wait_for_everyone()
        
        for cfg in GUIDANCE_SCALES:
            output_dir = f"{args.output_dir}/cfg_{cfg}"
    ...
    if rank == 0:
        print(f"\n{'='*60}")
        print(f"EMA Inference Complete!")
