#!/usr/bin/env python3
"""
Visualization script for comparing Qwen and Flux models across training steps.

Layout:
- Rows: Different training steps (1, 1000, 2000, ..., 49999)
- Columns: For each of 12 samples: GT, Qwen, Flux (36 images per row)

Output: HTML file with horizontal scrolling
"""
...
# Output HTML file
OUTPUT_HTML = "/home/v-yuxluo/yuxuanluo/experiments/visualize/260122_compare_qwen_flux/visualization_qwen_flux_comparison.html"

# SAS Token for Azure Blob Storage (for GT images)
SAS_TOKEN = "?sv=2025-07-05&spr=https%2Chttp&st=2026-01-22T14%3A24%3A01Z&se=2026-01-29T14%3A30%3A00Z&skoid=4b98d1ff-397b-40e8-a04e-6d6cbbb1ee35&sktid=72f988bf-86f1-41af-91ab-2d7cd011db47&skt=2026-01-22T14%3A24%3A01Z&ske=2026-01-29T14%3A30%3A00Z&sks=b&skv=2025-07-05&sr=c&sp=racwdxltf&sig=vFEUn9OfRjgvatdVk1TFiVwbGejfUjrHFRyNu1jTWrc%3D"

# Base URL for Azure Blob
BLOB_BASE_URL = "https://mcgvisionflowsa.blob.core.windows.net/yuxuanluo/"
GT_BLOB_PATH = "ArXiV_filtered_stages/stage4_aspect_quantize_filter/768_pretrain/"
QWEN_BLOB_PATH = "experiments/visualize/260122_compare_qwen_flux/Qwen_50k_oldlatent/media/images/"
FLUX_BLOB_PATH = "experiments/visualize/260122_compare_qwen_flux/FLUXklein9Bbase_50k_oldlatent/media/images/"

# Local paths for generated images
QWEN_IMAGE_DIR = "/home/v-yuxluo/yuxuanluo/experiments/visualize/260122_compare_qwen_flux/Qwen_50k_oldlatent/media/images"
FLUX_IMAGE_DIR = "/home/v-yuxluo/yuxuanluo/experiments/visualize/260122_compare_qwen_flux/FLUXklein9Bbase_50k_oldlatent/media/images"

# GT images base path (local filesystem)
GT_BASE_PATH = "/home/v-yuxluo/yuxuanluo/ArXiV_filtered_stages/stage4_aspect_quantize_filter/768_pretrain"

# Training steps to visualize
STEPS = [1, 1000, 2000, 3000, 4000, 5000, 7000, 10000, 12000, 14000, 16000, 18000, 20000, 22000, 24000, 26000, 28000, 30000, 32000, 34000, 36000, 38000, 40000, 42000, 44000, 46000, 48000, 49999]

# Sample info: (sample_idx, year, arxiv_id, image_filename, prompt)
# Extracted from validation_prompts comments in config file
# Format: year/arxiv_id/image_filename
SAMPLES = [
    (0, "2025", "2508.18950", "2508.18950_0_img_SINGLE.png", 
    ...
    (7, "2025", "2509.08205", "2509.08205_1_img_SINGLE.png",
    ...
    (8, "2025", "2506.23207", "2506.23207_1_img_SINGLE.png",
...
def build_image_index(image_dir, prefix_patterns):
    """Build an index of all images: (sample_idx, step) -> filepath"""
    index = {}
    
    for filepath in glob.glob(os.path.join(image_dir, "*.png")):
        filename = os.path.basename(filepath)
        
        # Try to parse: validation_sample_0_10000_xxxx.png or sample_0_0_10000_xxxx.png
        # Pattern 1: validation_sample_{sample}_{step}_{hash}.png
        match = re.match(r'validation_sample_(\d+)_(\d+)_[a-f0-9]+\.png', filename)
        if match:
            sample_idx = int(match.group(1))
            step = int(match.group(2))
            index[(sample_idx, step)] = filepath
            continue
        
        # Pattern 2: sample_{sample}_0_{step}_{hash}.png (Flux format)
        match = re.match(r'sample_(\d+)_0_(\d+)_[a-f0-9]+\.png', filename)
        if match:
            sample_idx = int(match.group(1))
            step = int(match.group(2))
            index[(sample_idx, step)] = filepath
        ...
        # Pattern 3: sample_{sample}_{something}_{step}_{hash}.png
        match = re.match(r'sample_(\d+)_\d+_(\d+)_[a-f0-9]+\.png', filename)
        if match:
            sample_idx = int(match.group(1))
            step = int(match.group(2))
            if (sample_idx, step) not in index:  # Don't override
                index[(sample_idx, step)] = filepath
...
<style>
    ...
    .img-card.gt .img-label { color: #4CAF50; font-weight: bold; }
    .img-card.qwen .img-label { color: #2196F3; font-weight: bold; }
    .img-card.flux .img-label { color: #FF9800; font-weight: bold; }
    .prompt-text {
        font-size: 16px;
        color: #333;
        padding: 12px;
    ...
    .legend-color.gt { background: #4CAF50; }
    .legend-color.qwen { background: #2196F3; }
    .legend-color.flux { background: #FF9800; }
    .missing {
        opacity: 0.3;
    }
</style>
...
function syncScroll(value) {
    ...
    const maxScroll = containers[0] ? containers[0].scrollWidth - containers[0].clientWidth : 0;
    const scrollPos = (value / 100) * maxScroll;
    containers.forEach(container => {
        container.scrollLeft = scrollPos;
    });
    // Update display: show sample number
    const sampleNum = Math.floor((value / 100) * 12);
    document.getElementById('sliderValue').textContent = 'Sample ' + sampleNum + ' / 11';
}
...
def main():
    print("Building image indices...")
    
    # Build indices for both model outputs
    qwen_index = build_image_index(QWEN_IMAGE_DIR, ["validation_sample"])
    flux_index = build_image_index(FLUX_IMAGE_DIR, ["sample"])
    
    print(f"Qwen images indexed: {len(qwen_index)}")
    print(f"Flux images indexed: {len(flux_index)}")
    
    # Debug: print some sample keys
    print("\nSample Qwen keys:", list(qwen_index.keys())[:5])
    print("Sample Flux keys:", list(flux_index.keys())[:5])
...
<head>
<meta charset="UTF-8">
<title>Qwen vs Flux Training Comparison</title>
{CSS_STYLE}
</head>
<body>
<h1>🎨 Qwen (50k) vs Flux-Klein-9B (50k) Training Comparison</h1>
<h2>Old Latent Version | 12 Validation Samples Across Training Steps</h2>

<div class="slider-container">
    <label>📍 Navigate Samples:</label>
    <input type="range" id="globalSlider" min="0" max="100" value="0" oninput="syncScroll(this.value)">
    <span id="sliderValue">Sample 0 / 11</span>
</div>

<div class="legend">
    <div class="legend-item"><div class="legend-color gt"></div> Ground Truth</div>
    <div class="legend-item"><div class="legend-color qwen"></div> Qwen</div>
    <div class="legend-item"><div class="legend-color flux"></div> Flux-Klein-9B</div>
</div>
""")
    
    # Generate rows for each step
    for step in STEPS:
...
""")
        ...
        # For each sample (now with 5 elements: sample_idx, year, arxiv_id, gt_filename, prompt)
        for sample_idx, year, arxiv_id, gt_filename, prompt in SAMPLES:
            gt_url = get_gt_blob_url(year, arxiv_id, gt_filename)
            
            # Get Qwen image - use Azure Blob URL
            qwen_path = qwen_index.get((sample_idx, step))
            if qwen_path:
                qwen_filename = os.path.basename(qwen_path)
                qwen_url = get_qwen_blob_url(qwen_filename)
                qwen_class = "qwen"
            else:
                qwen_url = ""
                qwen_class = "qwen missing"
            
            # Get Flux image - use Azure Blob URL
            flux_path = flux_index.get((sample_idx, step))
            if flux_path:
                flux_filename = os.path.basename(flux_path)
                flux_url = get_flux_blob_url(flux_filename)
                flux_class = "flux"
            else:
                flux_url = ""
                flux_class = "flux missing"
        ...
        <div class="sample-group">
            <div style="display:flex;flex-direction:column;">
                <div style="display:flex;gap:2px;">
                    <div class="img-card gt">
                        <a href="{gt_url}" target="_blank"><img src="{gt_url}" loading="lazy" alt="GT {sample_idx}"></a>
                        <div class="img-label">S{sample_idx} GT</div>
                    </div>
                    <div class="img-card {qwen_class}">
                        {"<a href='" + qwen_url + "' target='_blank'><img src='" + qwen_url + "' loading='lazy' alt='Qwen " + str(sample_idx) + "'></a>" if qwen_path else "<div style='height:150px;display:flex;align-items:center;justify-content:center;color:#666;'>Missing</div>"}
                        <div class="img-label">S{sample_idx} Qwen</div>
                    </div>
                    <div class="img-card {flux_class}">
                        {"<a href='" + flux_url + "' target='_blank'><img src='" + flux_url + "' loading='lazy' alt='Flux " + str(sample_idx) + "'></a>" if flux_path else "<div style='height:150px;display:flex;align-items:center;justify-content:center;color:#666;'>Missing</div>"}
                        <div class="img-label">S{sample_idx} Flux</div>
                    </div>
                </div>
                <div class="prompt-text"><b>S{sample_idx}:</b> {prompt_escaped}</div>
            </div>
        </div>
""")
...
""")
    ...
    print(f"   All images use Azure Blob URLs with SAS token")
